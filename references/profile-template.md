---
voiceprint_version: 0.3.0
created: 2026-05-01
last_review: 2026-05-01
review_threshold: 5
auto_prompt_intro_shown: false
---

# Voice profile

This file holds your **cross-register voice rules** — the things that are true about how you write, regardless of whether it's a LinkedIn post, an email, or a blog draft. Register-specific notes live in `registers/<name>.md` instead.

The humanizer floor (em-dash removal, banned vocabulary, generic-warmth fixes) is applied automatically from the skill itself on every draft. You don't need to copy it here. Only your own additions go in this file.

Voiceprint adds to the sections below over time, when you approve patterns during a review. You're welcome to edit anything by hand — it's your file.

## Cross-register notes

Voice rules that apply across every register. Approved review patterns destined for "core profile" land here.

## Things to avoid

Words, phrases, or moves that don't sound like you, regardless of register.

## Voice anchors

Short snippets, phrases, or one-liners that capture how you sound at your best. Reference points for voiceprint when generating drafts.
